	...
	this.makeDeck = function () {
		...
		for (var i = 0; i < cardSuits.length; i++) {								// for each suit
			...
			for (var j = 0; j < cardValues.length; j++) {							// and each value
				imgFace = imgLink + cardFaces[j] + cardSuits[i]+".png";				// create a new playing card
				var playingCard = new Card (cardSuits [i], cardValues [j], cardFaces [j], imgFace );	// use the same index for face and value. they should be associated.	
